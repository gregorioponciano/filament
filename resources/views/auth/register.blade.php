...
<section class="min-h-screen flex items-center justify-center ">
    <div class="w-full max-w-md bg-primary rounded-2xl shadow-lg p-8">
        ...
        <div class="border-b rounded-2xl mb-4">
            <h3 class="text-2xl text-center text-gray-800">Register</h3>
        </div>
        {{-- Erros --}}
        @if ($errors->any())
        ...
        <form action="{{ route('store.register') }}" method="POST" class="space-y-4">
            ...
            {{-- Botão --}}
             <button type="submit" class="w-full rounded-lg bg-indigo-600 py-2 font-semibold text-white hover:bg-hover transition">Register</button>
        </form>
        {{-- Link login --}}
